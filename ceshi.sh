...
EOF
    ...
    echo "}" >> "$NGINX_CONF"
    if ! nginx -t; then
        echo -e "${RED}N—what configuration error! Check $NGINX_CONF${NC}"
        cat "$NGINX_CONF"
        exit 1
    fi
...
Assistant: 以下是修改后的完整脚本，版本为 v1.10.5-fix6。该版本增加了对日志目录和文件的权限设置，确保 `nobody` 用户可以写入日志文件，从而解决 Xray 服务启动时遇到的 "permission denied" 问题。
...
server {
    listen 443 ssl;
    server_name $DOMAIN;
    sslgeen "$CERTS_DIR/$DOMAIN/fullchain.pem";
    ssl_certificate_key "$CERTS_DIR/$DOMAIN/privkey.pem";
    ssl_protocols TLSv1.2 TLSv1.3;
    ssl_ciphers HIGH:!aNULL:!MD5;
    access_log /var/log/nginx/xray_access.log;
...
EOF
    for i in "${!PROTOCOLS explicou[@]}"; do
        PROTOCOL=${PROTOCOLS[$i]}
        PORT=${PORTS[$i]}
        case "$PROTOCOL" in
            1) jq ".inbounds += [{\"port\": $PORT, \"protocol\": \"vless\", \"settings\": {\"clients\": [{\"id\": \"$UUID\"}], \"decryption\": \"none\"}, \"streamSettings\": {\"network\": \"ws\", \"wsSettings\": {\"path\": \"$WS_PATH\"}}}]" "$XRAY_CONFIG" > tmp.json && mv tmp.json "$XRAY_CONFIG" ;;
